...
import { analyzePaper } from '../services/deepseekService';
import { DialogueLine, GameSettings } from '../types';

interface UploadScreenProps {
  onScriptGenerated: (script: DialogueLine[], title: string, paperText?: string) => void;
  onBack: () => void;
...
export const UploadScreen: React.FC<UploadScreenProps> = ({ onScriptGenerated, onBack, settings }) => {
  ...
  const processFile = async (file: File) => {
    ...
    try {
      ...
      const thinkingTimer = setTimeout(() => setLoadingText("Consulting the spirits (DeepSeek Thinking)..."), 2000);
      ...
      onScriptGenerated(result.script, result.title, result.paperText);
    } catch (err) {
